...
interface LogcatLogger {
     ...
     */
    fun log(
        priority: LogPriority, tag: String, message: String
    )

    fun logFile(
        priority: LogPriority, logFile: File, message: String
    )
            ...
            synchronized(this) {
                if (isInstalled) {
                    logger.log(
                        LogPriority.ERROR,
                        "LogcatLogger",
                        "Installing $logger even though a logger was previously installed here: " + installedThrowable!!.asLog()
                    )
                }
